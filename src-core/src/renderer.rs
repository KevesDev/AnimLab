...
use crate::math;
use crate::geometry::{self, VectorElement};
use crate::graph::{AnimGraph, NodeId};
...
pub struct WebGpuRenderer {
    ...
    pub raster_cache: HashMap<NodeId, (wgpu::Texture, wgpu::BindGroup)>,
...
impl WebGpuRenderer {
    pub async fn new(canvas: HtmlCanvasElement, physical_width: u32, physical_height: u32) -> Result<Self, String> {
        ...
        // AAA FIX: Use explicit String mapping to bypass E0282 and E0599 type inference failures
        let surface = instance.create_surface(wgpu::SurfaceTarget::Canvas(canvas))
            .map_err(|_| String::from("Failed to create WebGPU surface"))?;
            
        let adapter = instance.request_adapter(&wgpu::RequestAdapterOptions { 
            power_preference: wgpu::PowerPreference::HighPerformance, 
            compatible_surface: Some(&surface), 
            force_fallback_adapter: false 
        }).await.map_err(|_| String::from("Failed to request WebGPU adapter"))?;
        ...
        let (device, queue) = adapter.request_device(&wgpu::DeviceDescriptor { 
            ...
            ..Default::default() 
        }).await.map_err(|_| String::from("Failed to request WebGPU device"))?;
        ...
        Ok(Self {
            ...
            standard_pipeline, stencil_write_pipeline, stencil_read_0_pipeline, stencil_read_1_pipeline, depth_stencil_texture,
            raster_pipeline, raster_bind_group_layout, raster_cache: HashMap::new(), quad_vb, quad_ib,
        })
    }
    ...
    pub fn render(&mut self, graph: &AnimGraph, active_tool: &dyn CanvasTool, canvas_width: f32, canvas_height: f32) {
        let output = match self.surface.get_current_texture() { Ok(frame) => frame, Err(wgpu::SurfaceError::Outdated) => return, Err(e) => { error!("Surface error: {:?}", e); return; } };
        ...
        
        {
            ...
            let render_list = graph.collect_renderable_elements();
            ...
            let target_node_id = graph.active_layer_node.unwrap_or(1);
            if let Some(aabb) = graph.get_selection_aabb(target_node_id) {
                ...
                let (box_v, box_i, _) = geometry::tessellator::Extruder::extrude_centerline(&pts, 1.0, tb_orange, canvas_width, canvas_height);
                ...
                let mut add_quad = |x: f32, y: f32, w: f32, h: f32, color: [f32; 4]| {
                    let left = (x / canvas_width) * 2.0 - 1.0; let right = ((x + w) / canvas_width) * 2.0 - 1.0;
                    let top = 1.0 - (y / canvas_height) * 2.0; let bottom = 1.0 - ((y + h) / canvas_height) * 2.0;
                ...
                let (px, py) = active_tool.get_custom_pivot().unwrap_or((
            ...
            let preview_mesh_result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                active_tool.get_preview_mesh(canvas_width, canvas_height)
            }));
            
            let (preview_verts, preview_inds) = match preview_mesh_result {
                Ok(mesh) => mesh,
                Err(_) => { error!("AAA Safety Net: Handled rendering panic in get_preview_mesh."); (Vec::new(), Vec::new()) }
            };

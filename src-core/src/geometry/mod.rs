...
pub struct EraserMask { 
    pub shape: MultiPolygon<f32>, // AAA FIX: Stores the math shape for precise hit-testing
...
impl VectorElement {
    ...
    pub fn translate(&mut self, dx: f32, dy: f32, canvas_width: f32, canvas_height: f32) {
        ...
        let clip_dy = -(dy / canvas_height) * 2.0;
        ...
        match self {
            ...
            VectorElement::Contour(s) => {
                ...
                for mask in &mut s.eraser_masks { 
                    mask.shape.translate_mut(dx, dy); // Keeps the hit-box perfectly aligned
                    for v in &mut mask.vertices { v.position[0] += clip_dx; v.position[1] += clip_dy; } 
                }

...
impl AnimLabEngine {
    ...
    pub async fn attach_canvas(&mut self, canvas: HtmlCanvasElement, physical_width: u32, physical_height: u32) -> Result<(), JsValue> {
        ...
        let render_pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor { label: Some("Render Pipeline Layout"), bind_group_layouts: &[], immediate_size: 0 });
        
        let render_pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("Main Render Pipeline"), layout: Some(&render_pipeline_layout),
    ...
    pub fn resize_surface(&mut self, physical_width: u32, physical_height: u32) {
        ...
        if let (Some(device), Some(surface), Some(config)) = (&self.device, &self.surface, &mut self.config) {
            config.width = safe_width;
            config.height = safe_height;
            surface.configure(device, config);
        }
    }
    ...
    pub fn end_stroke(&mut self) -> Result<(), JsValue> {
        if let Some(mut stroke) = self.active_stroke.take() {
            ...
            // AAA UPGRADE: Target the active layer (which is now properly bifurcated)
            let target_node_id = self.graph.active_layer_node.expect("Fatal Engine Error: Missing active layer in DAG.");
        ...
        if let (Some(device), Some(queue), Some(surface), Some(pipeline)) = (&self.device, &self.queue, &self.surface, &self.render_pipeline) {
            ...
            
            {
                ...
                let mut draw = |stroke: &Stroke| {
                    ...
                    let vb = device.create_buffer_init(&wgpu::util::BufferInitDescriptor { label: Some("Vertex Buffer"), contents: bytemuck::cast_slice(&stroke.vertices), usage: wgpu::BufferUsages::VERTEX });
                    let ib = device.create_buffer_init(&wgpu::util::BufferInitDescriptor { label: Some("Index Buffer"), contents: bytemuck::cast_slice(&stroke.indices), usage: wgpu::BufferUsages::INDEX });

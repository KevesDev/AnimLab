...
export const TimelinePanel: React.FC = () => {
    ...
    // AAA FIX: Dynamically render widths based on actual data limits
    ...
    return (
        <div style={{ display: 'flex', width: '100%', height: '100%', backgroundColor: '#1c1d20', userSelect: 'none', fontFamily: 'sans-serif', overflow: 'hidden' }}>
            ...
            <div style={{ width: '250px', minWidth: '250px', borderRight: '1px solid #111', display: 'flex', flexDirection: 'column', backgroundColor: '#222327', flexShrink: 0, zIndex: 30 }}>
                ...
                {/* AAA FIX: The Timeline Toolbar (Add/Delete/Limits) */}
                <div style={{ height: '32px', minHeight: '32px', borderBottom: '1px solid #111', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 12px', backgroundColor: '#1c1d20', boxSizing: 'border-box' }}>
                    ...
                    <div style={{ display: 'flex', gap: '4px' }}>
                        <button onClick={() => { if(timelineLayers.length < 100) setAddLayerModalOpen(true); }} style={{ background: 'transparent', border: 'none', cursor: timelineLayers.length < 100 ? 'pointer' : 'not-allowed', color: timelineLayers.length < 100 ? '#bbb' : '#444', fontSize: '14px', fontWeight: 'bold' }} title={timelineLayers.length < 100 ? "Add Layer" : "100 Layer Limit Reached"}>+</button>
                ...
                <div style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden' }}>
                    ...
                    {timelineLayers.map((element) => {
                        ...
                        return (
                            <div 
                                key={element.id.toString()} onClick={() => store.setSelectedLayerId(element.id)}
                                    ...
                                    .map(block => {
                                        ...
                                        return (
                                            <div 
                                                ...
                                                style={{ 
                                                    position: 'absolute', display: 'flex', alignItems: 'center', zIndex: 10, boxSizing: 'border-box', left: `${(block.start - 1) * frameWidth}px`, width: `${block.duration * frameWidth}px`, height: '16px', top: '2px', backgroundColor: '#c4c4c4', borderTop: '1px solid #ffffff', borderLeft: '1px solid #ffffff', borderBottom: '1px solid #777777', borderRight: '1px solid #777777', borderRadius: '3px', cursor: 'grab', opacity: isGhosting ? 0 : 1 
                                                }}
                                            >
                                                {controller && !isGhosting && <div onPointerDown={(e) => controller.handleBlockPointerDown(TimelineMode.ExtendingLeft, e, element.id, block.start, block.duration)} style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: '4px', cursor: 'ew-resize', zIndex: 20 }} />}
                                                {controller && !isGhosting && <div onPointerDown={(e) => controller.handleBlockPointerDown(TimelineMode.ExtendingRight, e, element.id, block.start, block.duration)} style={{ position: 'absolute', right: 0, top: 0, bottom: 0, width: '4px', cursor: 'ew-resize', zIndex: 20 }} />}
                                            </div>
                                        );
                                    })
                                }
